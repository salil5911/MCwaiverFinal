...
const formSchema = z.object({
  invoice_number: z.string().min(1, "RMA number is required"),
  vendor_name: z.string().min(1, "Vendor name is required"),
  invoice_date: z.date({
    required_error: "RMA date is required",
  }),
...
export function RmaForm({ location, onSuccess }: RmaFormProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const { error } = await supabase.from("invoice_sheets").insert({
        ...
        is_rma: true,
        rma_status: "RMA Sent – Waiting for Approval",
      })
      ...
      toast({
        title: "RMA added successfully",
        description: "The RMA has been added to the system.",
      })
      ...
      toast({
        title: "Error adding RMA",
        description: "There was an error adding the RMA. Please try again.",
        variant: "destructive",
      })
    } finally {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => setOpen(true)}>
          <Plus className="mr-2 h-4 w-4" /> Add RMA
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle>Add New RMA</DialogTitle>
          <DialogDescription>Enter the RMA details for {location} location.</DialogDescription>
        </DialogHeader>
        <div
